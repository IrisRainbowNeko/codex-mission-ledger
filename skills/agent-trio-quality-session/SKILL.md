...
For each new foreground run, generate a UUID-style `runId`, call `action=submit` with
`monitorFirst=true`, then make exactly one `action=status` call with that ID and `wait=true`. The MCP Apps monitor mounts while work runs; do not poll. Resume an existing `waiting_input` run when the
user supplies its requested input.
