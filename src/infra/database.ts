...
export function mapSqliteError(error: unknown, databasePath: string): unknown {
  ...
  if (
    ...
    return new ControlPlaneError(
      "forbidden",
      `Control plane SQLite is not writable at ${databasePath}: ${message}. Codex sandboxes often treat ~/.codex as read-only; Mission Ledger for Codex stores the ledger under %LOCALAPPDATA%\\codex-mission-ledger on Windows, ~/.local/share/codex-mission-ledger elsewhere, or a temp directory.`,
      { databasePath, sqlite: message, sqliteCode: code },
    );
  }
  return error;
