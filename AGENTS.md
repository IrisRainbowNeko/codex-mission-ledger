...
8. Sol never polls with `list_agents`, `wait`, `send_message`, or `sleep`. One `wait_agent` at 1h. After timeout, `children_status` once — do not wait 1h three times. Cancel `leaseExpired` **running** tasks; never cancel parked `blocked` jobs. If a child is `blocked` (training/remote), stop and tell the user to **继续** — do not `mission_close`. Terra never babysits children; one long `wait_agent` then `children_status`. On fanout, Sol closes from the Terra summary only when nothing is parked. Default risk is `medium` unless the user asked for irreversible damage. Sol's last user-visible reply includes the path Luna wrote.
